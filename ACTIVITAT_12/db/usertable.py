...
def edit(user_id: str, punts_actuals: int, total_partides: int, win_partides: int, record_time: str, record_punts: int):
    conn = get_db_connection()
    cur = conn.cursor()
    try:
        cur.execute(
            'UPDATE usertable '
            'SET punts_actuals = %s, total_partides = %s, win_partides = %s, record_time = %s, record_punts = %s '
            'WHERE "user" = %s', (punts_actuals, total_partides, win_partides, record_time, record_punts, user_id)
        )
        conn.commit()
